module n2ext
  ...
  private
    real, parameter :: pi151 = 1.0*pi**(-1.5)

    real, parameter :: n2C(3,2) = reshape((/ -0.261008192263701 + 1.0/sqrt(pi), 1.0/pi, pi151,&
                                  -0.261008192263701 + 1.0/sqrt(pi), 1.0/pi, pi151 /),(/3,2/))
 ...
 contains
  ...
  subroutine n2f(r, h, f)
    ...
    if (q < 0.0) then
      ...
      stop
    elseif (q < 2.0) then
      f  = 0.0183127964683465*q**3 - 0.174477294027598*q**2 - 1.7053968728626432*q &
       - (-2.2035977856942233*q + 1.8169193558843038)*erf(1.5690821350931794*q - 1.2937459461663592) &
       + 0.14859319297412515*exp(q*(-2.4620187466685706*q + 4.059987302957713)) + 1.369093749870065
    else
      f = .0
    end if
  end subroutine n2f
