...
!  OWNER: Daniel Price
!
!  $Id: e439ec6d7c32bb2b60e8bed2672c62776908dc8c $
!
...
!
 ...
 if (totmass_node > 0.) then
    xcofm = xcofm/(totmass_node*dfac)
    ycofm = ycofm/(totmass_node*dfac)
    zcofm = zcofm/(totmass_node*dfac)
 endif

#ifdef MPI
 ! if this is global node construction
 if (present(groupsize)) then
    call get_group_cofm([xcofm,ycofm,zcofm],totmass_node,level,xyzcofmg,totmassg)
    xcofm = xyzcofmg(1)
    ycofm = xyzcofmg(2)
    zcofm = xyzcofmg(3)
...
!--for gravity, we need the centre of the node to be the centre of mass
 ...
 if (.not. wassplit) then
    ...
    if (npnode > 0) then
       ...
       do while((j>=nl).and.(xyzh_soa(j,iaxis) > xpivot))
         counterr = counterr+1
         j = nr-counterr
       end do

       do while(i < j)
         if (xyzh_soa(i,iaxis) <= xpivot) then
           counterl = counterl+1
           i = nl+counterl
         else
       ...
       inoderange(1,il) = nl
       inoderange(2,il) = nl + counterl - 1
       inoderange(1,ir) = nr - counterr + 1
       inoderange(2,ir) = nr
       ! print*, i, j, nl, nr, counterl, counterr
       ! print*, inoderange(1,il), inoderange(2,il), inoderange(1,ir), inoderange(2,ir)
       ! print*, xyzh_soa(nl:i,iaxis) <= xpivot
       ! print*, xyzh_soa(i+1:nr,iaxis) <= xpivot
       ! read*
       nl = counterl
       nr = counterr
