...
contains
  ...
  subroutine c2(store, maxconsenrg)
    ...
    !$omp private(rab, dr, ra, rb, vab, urab, rhoa, rhob, nwa, nwb, fab)&
      ...
      overb: do lb = 1, size(nlistb)
        ...
        if (doDiffusion == 1) then
          ...
          ! else if ((s_ktp == esd_fw).or.(s_ktp == esd_fab)) then
            ...
            ! qc = abs(ta - tb) * 0.5 * (dot_product((nwa(:)/rhoa/rhoa + nwb(:)/rhob/rhob),urab(:)))
            ! call Fab_cart(rab, ha, fab)
            ! qc = mb / ((rhoa + rhob) / 2)**2 * dot_product(kdtadx(:) - kdtbdx(:),urab(:))*fab
            ! print*, qc, ddta
            ! read*
            ! ddta = ddta + qc
            else
