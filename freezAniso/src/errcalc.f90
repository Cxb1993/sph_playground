...
contains
  ...
  subroutine diffshock12(store, t, err)
    real, allocatable, intent(in)    :: store(:,:)
    real, allocatable, intent(inout) :: err(:)
    real, intent(in)                 :: t
    ...
    real :: &
      tr, tl, x, k, exact, kappa

    tr = 2.
    tl = 1.

    call getNeibListL1(nlista)
    call getdiffconductivity(kappa)

    err(:) = 0.
    do j = 1,size(nlista)
      i = nlista(j)
      x = store(es_rx,i)
      k  = store(es_bx,i)*kappa
      exact = (tr + tl)/2. + (tr - tl)/2. * erf(x/sqrt(4.*k*t))
      err(i) = (exact - store(es_t,i))**2
    end do
  end subroutine
